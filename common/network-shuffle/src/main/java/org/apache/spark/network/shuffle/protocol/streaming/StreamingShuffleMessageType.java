 ...
 * message body, so this enum is what lets a received frame be routed to the correct decoder.
 * The byte is written as the framing prefix by {@code StreamingShuffleMessage.toByteBuffer()}
 * and consumed by {@code StreamingShuffleMessage.Decoder.fromByteBuffer(ByteBuffer)}.
 *
 ...
 *
 * This is a deliberately separate, parallel id space from the
 * {@link org.apache.spark.network.shuffle.protocol.BlockTransferMessage.Type} enum used by the
 * block-transfer and push-based-shuffle families: streaming shuffle neither extends nor
 * interoperates with them, so its ids start again at 0. The two enums are never mixed on a
 * single channel, which is why the overlap in numeric values is harmless. Because these are
 * wire values they are frozen: an existing constant must never be renumbered, and a new message
 * type must take the next unused id, so that a peer built against a different revision fails
 * fast with an explicit error instead of silently misreading a frame.
 *
 * @since 4.2.0
 */
public enum StreamingShuffleMessageType {
   ...
   *
   * This accessor is deliberately public and must remain so. Unlike
   * {@link org.apache.spark.network.shuffle.protocol.BlockTransferMessage.Type}, which is nested
   * inside the class that encodes it and can therefore read the private field directly, this
   * enum is a top-level type in its own file, so the encoder reaches the wire value through
   * {@code type().id()}.
   *
  ...
  public static StreamingShuffleMessageType fromId(byte id) {
    // A switch rather than a scan over values(): values() defensively clones its backing array
    // on every call, and this method runs once per received frame on the shuffle hot path.
    return switch (id) {
      case 0 -> DATA_BLOCK;
      case 1 -> ACK;
      case 2 -> HEARTBEAT;
